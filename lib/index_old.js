...
exports.compileModule = function(filename, src, language, options, callback) {
  ...
  var moduleFilename = path.relative(path.dirname(filename), filename);
  var moduleName = moduleFilename.replace(/\.vgl$/, '');
  var namespace = options.namespace || path.dirname(path.resolve(filename)).split(path.sep);
  if (namespace instanceof Array) {
    namespace = namespace[namespace.length - 1];
  }

  var world = new World(filename, options);
  world.parseString(src, moduleFilename, function(err) {
    if (err) {
      err.world = world;
      return callback(err);
    }
    ...
    try {
      ...
      if (options.prune) {
        prune = {
          rootModule: world.modules[moduleFilename],
          entryPoints: options.prune.entryPoints
        };
      }
      passes.runAll(world, {prune: prune});
