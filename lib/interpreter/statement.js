...
var executeStatement = module.exports = function statement(node, slots) {
  ...
  switch (node.constructor) {
    ...
    case ast.ForStatement:
      throw new Error('This is still TODO - swap out the MutVarDecl for a Slot');
      var d = node.declaration;
      var cur = executeExpression(d.expression, slots).value.value;
      var end = executeExpression(d.end, slots).value.value;
      var status = new ast.MutableVariableDeclaration(d.name, d.type, cur);
      ...
      while (true) {
        ...
        var result = executeBlock(node.body, slots, [[d.name, status]]);
        if (result) {
          if (result.type == 'return') {
            return result;
          } else if (result.type == 'continue') {
