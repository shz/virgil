...
var grabExports = function(ownerScope, importNode) {
  ...
  if (!importNode.ast) {
    importNode.throw('Compiler bug: this import node has no ast');
  }
    ...
    var check = function(node) {
      ...
      if (node.constructor == ast.MethodStatement) {
        if (cur.findMethod(node.args[0][1], node.name))
          node.throw('Method "' + node.name + '" is already defined on ' +
            node.args[0][1].toString() + ' in this scope');
        else
          return;
      }
      ...
      if (defScope) {
        if (defScope == cur)
          node.throw('Name "' + node.name + '" is already defined in this scope');
         // Only allow overriding regular old var decls (not loop vars)
        if (node.override && !defScope.variables[node.name].from)
          return;
        if (node instanceof ast.VariableDeclaration ||
          ...
          node.throw('Name "' + node.name + '" is already defined in a parent scope. ' +
            'Try using ! if you want to override (e.g. let! foo = 1)');
        }
    ...
    switch (n.constructor) {
      ...
      case ast.ForStatement:
        ...
        try {
          ...
          n.throw('Name "' + n.declaration.name + '" is already defined');
        }
      ...
      // Resolve all identifiers to ensure they're in scope
      case ast.Identifier:
        if (!cur.search(n.name)) {
          n.throw(n.name + ' is not in scope');
        }
        break;
    }
