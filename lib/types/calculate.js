...
//        can make use of it.  Also remove the ability to defined
  ...
  switch (c) {
    ...
    case ast.ListAccessExpression:
      ...
      if (!exp.type.builtin && !exp.type.isGeneric) {
        ...
        if (!defScope) {
          doThrow('Unknown struct ' + exp.type.name, exp);
        }
    ...
    case ast.PowerExpression:
      ...
      if (!left.numeric)
        doThrow('Left side of power operator must be numeric, but is ' + left.toString(), exp);
      if (right != canned['int'])
        doThrow('Right side of power operator must be an int, but is ' + right.toString(), exp);
    ...
    case ast.LogicalAndExpression:
      ...
      if (!equal(left, right)) {
        doThrow('Both possible results from a binary expression must ' +
          'resolve to the same type (got ' + left.toString() + ' and ' + right.toString() + ')', exp);
      }
      exp.type = left;
      return exp.type;
    ...
    case ast.FunctionCallExpression:
      ...
      var argTypes = exp.args.map(function(a, i) {
        ...
        if (a.constructor == ast.LambdaExpression) {
          a.inferredTypes = funcTypes[i].generics;
        }
        return calculate(a);
      });
      ...
      if (!exp.type.builtin && !exp.type.isGeneric) {
        var defScope = exp.scope.search('struct', exp.type.name);
        if (!defScope) {
          doThrow('Unknown type ' + exp.type.name, exp);
        }
        exp.type.def = defScope.structs[exp.type.name];
      }
    ...
    case ast.PropertyAccessExpression:
      ...
      if (left == canned['void']) {
        doThrow('Cannot access property ' + exp.right.name + ' on void', exp);
      }
      ...
      // Handle user types
        ...
        if (!defScope) {
          ...
          doThrow('Compiler bug: no struct named "' + left.name + '".  How do we know it\'s a struct?', exp.left);
        }
    ...
    switch (n.constructor) {
      ...
      case ast.OutVariableDeclaration:
        ...
        if (!n.type.builtin && !n.type.isGeneric && !n.type.def) {
          ...
          if (!defScope) {
            doThrow('No struct named ' + n.type.name, n);
          }
      ...
      case ast.AssignmentBlock:
        n.declarations.forEach(function(d) {
          ...
          if (d.type == canned['inferred']) {
            if (t == canned['inferred']) {
              doThrow('Property ' + d.name + ' cannot be initialized to default because no type is declared', d.expression);
            }
            if (t == canned['null']) {
              doThrow('Property ' + d.name + ' cannot be initialized to null because no type is declared', d.expression);
            }
            d.type = t;
          } else if (t != canned['inferred'] && t != canned['null'] && !equal(d.type, t)) {
            doThrow('Property ' + d.name + ' is declared as type ' + d.type + ' but is defined as ' + t, d.expression);
          } else if (t == canned['inferred']) {
            d.expression.type = d.type;
          }

          if (!d.type) {
            doThrow('Compiler error, node does not have a type somehow');
          }
        });
        break;
      ...
      case ast.NewExpression:
        ...
        if (n.type.builtin) {
          if (n.args.declarations.length) {
            doThrow('Type ' + n.type + ' has no attribute ' + n.args.declarations[0].name);
          }
          break;
        }
        ...
        var defScope = n.scope.search('struct', n.type.name);
        if (!defScope) {
          doThrow('Type ' + n.type.name + 'is undefined', n);
        }
        var structNode = defScope.structs[n.type.name];
        if (structNode.generics.length != n.type.generics.length) {
          doThrow('Type ' + n.type.name + ' expects ' + structNode.generics.length + ' type parameters, but was passed ' + n.type.generics.length, n);
        }
        ...
        n.args.declarations.forEach(function(d) {
          ...
          if (!typeStruct) {
            doThrow('Type ' + n.type + ' has no property ' + d.name, d);
          }
          var typeUsed = calculate(d.expression);
          if (!equal(typeUsed, typeStruct)) {
            doThrow('Property ' + d.name + ' of ' + n.type + ' is type ' + typeStruct + ', but is being initialized with ' + typeUsed, d);
          }
        });
        break;
