  ...
  switch (c) {
    ...
    case ast.FunctionCallExpression:
      ...
      if (t.hasGenericReference()) {
        try {
          var resolution = generics.resolve( new TypeRef('func', funcTypes)
                                           , new TypeRef('func', argTypes)
                                           );
          if (exp.type.isGeneric && resolution.hasOwnProperty(exp.type.name)) {
            // console.log('updating return type', exp.type.name, 'to', resolution[exp.type.name].toString());
            exp.type = resolution[exp.type.name];
          }
        } catch (err) {
      ...
      // Handle user types
        ...
        for (var i=0; i<structNode.body.declarations.length; i++) {
          ...
          if (decl.name == exp.right.name) {
            ...
            // Get specific for generic types
            if (exp.type.isGeneric) {
              exp.type = left.generics[structNode.generics.indexOf(exp.type.name)]
            }
            return exp.type;
          }
        }
...
var realize = exports.realize = function(root) {
  ast.traverse(root, function(n) {
    switch (n.constructor) {
      ...
      // Functions, methods, and lambdas need their args assigned
      // types.
      // case ast.FunctionStatement:
      // case ast.MethodStatement:
      // case ast.LambdaExpression:
      //   for (var i=0; i<n.args.length; i++) {
      //     console.log(n.body.scope.variables);
      //     n.body.scope.variables[n.args[i][0]].type = n.args[i][1];
      //   }
      //   break;
