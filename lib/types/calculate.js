        ...
        if (n.type.builtin) {
          if (n.type == canned['datetime']) {
            ...
            n.args.declarations.forEach(function(d) {
              var typeStruct = null;
              // Hunt for the property
              //console.log(builtin['datetime'].attributes);
              //console.log(Object.keys(builtin['datetime'].attributes));
              var keys = Object.keys(builtin['datetime'].attributes);
              for (var i=0; i<keys.length; i++) {
                if (keys[i] == d.name) {
                  var decl = builtin['datetime'].attributes[keys[i]];
                  //console.log('*************');
                  //console.log(decl);
                  typeStruct = decl;
