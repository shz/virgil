...
var calculate = exports.calculate = function(exp) {
  ...
  switch (c) {
    ...
    case ast.LessThanEqualExpression:
      var l = calculate(exp.left);
      var r = calculate(exp.right);

      if (!l.numeric || !r.numeric) {
        exp.throw('Both sides of a comparison must be numeric types; got ' + l.toString() + ', ' + r.toString());
      }
    ...
    case ast.NotEqualsExpression:
      ...
      var r = calculate(exp.right);
      if (!equal(l, r)) {
        exp.throw('Left and right sides must be of the same type; got ' + l.toString() + ', ' + r.toString());
      }
    ...
    case ast.NotExpression:
      ...
      if (!equal(canned['bool'], t)) {
        exp.throw('Negation may only be applied to bools; got ' + t.toString());
      }
      exp.type = canned['bool'];
      return exp.type;
    ...
    case ast.DivisionExpression:
      ...
      if (!left.numeric || !right.numeric) {
        ...
        var message = 'Arithmetic may only be performed on numeric types, got ' + left + ', ' + right;
    ...
    case ast.TernaryExpression:
      ...
      if (cond != canned['bool']) {
        exp.condition.throw('Ternary conditions must evaluate to a bool, got ' + cond.toString() + ' instead');
      }
      // FALL THROUGH
    ...
    case ast.LogicalAndExpression:
      ...
      if (!equal(left, right)) {
        exp.throw('Both possible results from a ' +
          (c == ast.TernaryExpression ? 'ternary' : 'binary') + ' expression must ' +
    ...
    case ast.FunctionCallExpression:
      ...
      try {
        if (t.name == 'method') {
          resolution = generics.resolve(
            t,
            new TypeRef(t.name, [calculate(exp.left.left)].concat(exp.args.map(function(a) {
      ...
      var argTypes = exp.args.map(function(a, i) {
        ...
        var argType = calculate(a);
        if (a.constructor == ast.ListExpression && argType == canned['inferred']) {
          argType = a.type = funcTypes[i];
        }
        return argType;
      });
  ...
  ast.traverse(root, function(n) {
    switch (n.constructor) {
      ...
      case ast.AssignmentStatement:
        ...
        if (r == canned['inferred'] && l != canned['inferred']) {
          r = n.right.type = l;
        }
      ...
      case ast.ReturnStatement:
        ...
        if (n.type == canned['inferred']) {
          n.type = n.expression.type = curFunction.returnType;
        }
      ...
      case ast.AssignmentBlock:
        n.declarations.forEach(function(decl) {
          var exprType = calculate(decl.expression);

          if (decl.type == canned['inferred']) {
            if (exprType == canned['inferred']) {
              decl.expression.throw('Property ' + decl.name + ' cannot be initialized to default because no type is declared');
            }
            if (exprType == canned['null']) {
              decl.expression.throw('Property ' + decl.name + ' cannot be initialized to null because no type is declared');
            }
            // Infer type of declaration from expression
            decl.type = exprType;
          } else if (exprType != canned['inferred'] && !equal(decl.type, exprType)) {
            decl.expression.throw('Property ' + decl.name + ' is declared as type ' + decl.type + ' but is defined as ' + exprType);
          } else if (exprType == canned['inferred']) {
            decl.expression.type = decl.type;
          }
      ...
      case ast.NewExpression:
        ...
        if (n.type.builtin) {
          if (n.type == canned['datetime']) {
            // verify names and types of attribute-settings involved in this use of "new"
            n.args.declarations.forEach(function(d) {
              var attr = builtin['datetime'].attributes[d.name];

              if (!attr) {
                n.throw('Type ' + n.type + ' has no property ' + d.name);
              }
              var typeUsed = calculate(d.expression);
              if (!equal(typeUsed, attr)) {
                n.throw('Property ' + d.name + ' of ' + n.type + ' is type ' + attr.name + ', but is being initialized with ' + typeUsed);
              }
            });
          }else{
            n.throw('Cannot use "new ' + n.type.toString() + '"; try default keyword instead');
          }
          break;
        }
