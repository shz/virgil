...
var traverse = module.exports = function(node, parent, enter, exit) {
  ...
  if (node === null || node === undefined) {
    // Damn thing doesn't even exist!
    if (parent) {
      parent.throw('Attempted to traverse this node, but it has a null/undefined child');
    } else {
      throw new Error('Null or undefined node, with no parent information.  Madness.');
    }
  }
  ...
  if (!skip) {
    switch (node.constructor) {
      ...
      default:
        // Who knows what this crazy node is, don't rely on it having
        // a .throw() method available.
