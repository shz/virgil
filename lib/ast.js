...
inherit(DefaultLiteral, Expression);
...
inherit(NegationExpression, Expression);
...
inherit(NotExpression, Expression);
inherit(GreaterThanExpression, Expression);
inherit(LessThanExpression, Expression);
inherit(GreaterThanEqualExpression, Expression);
inherit(LessThanEqualExpression, Expression);
inherit(EqualsExpression, Expression);
inherit(NotEqualsExpression, Expression);
...
inherit(AssignmentBlock, Statement); // Changed from Expression?
...
var traverse = exports.traverse = function(node, enter, exit) {
  ...
  try {
    ...
    if (!err.start && !err.end && node && node.loc) {
