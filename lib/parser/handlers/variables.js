...
exports['mut'] = {
  nud: function(value, loc) {
    ...
    var mutvd = new syntax.MutableVariableDeclaration(node.name, node.type, node.expression);
    mutvd.loc = node.loc;
    return mutvd;
  }
};
...
exports['out'] = {
  nud: function(value, loc) {
    var node = exports['let'].nud.call(this, value, loc);
    var outvd = new syntax.OutVariableDeclaration(node.name, node.type, node.expression);
    outvd.loc = node.loc;
    return outvd;
  }
};
