...
var parseGlobalDecl = function(loc) {
  ...
  var dummy = new ast.NullLiteral();
  dummy.loc = name.loc;
  var node = new ast.VariableDeclaration(name.value, type, dummy, false);
...
var parseStructDelc = function(loc) {
  ...
  var openBrace = this.expect('{');
  while (!this.peek('}')) {
    var declName = this.expect('identifier');
    this.expect(':');
    var type = this.typeRef();

    var dummy = new ast.DefaultLiteral();
    dummy.loc = declName.loc;
    var decl = new ast.VariableDeclaration(declName.value, type, dummy, false);
  ...
  var assignments = new ast.AssignmentBlock(declarations);
  assignments.loc = {
    start: openBrace.loc.start,
    end: this.prev.loc.end
  };
  var node = new ast.StructStatement(name, assignments, false, generics);
  node.loc = { start: loc.start, end: assignments.loc.end };
  return node;
};
