...
var parseStructDelc = function(loc) {
  ...
  while (!this.peek('}')) {
    ...
    var decl = new ast.VariableDeclaration(declName[1], type, new ast.DefaultLiteral(), false);
    decl.loc = {
      start: {
...
exports['extern'] = {
  nud: function(value, loc) {
    ...
    if (this.peek('identifier', true))
      namespace = this.expect('identifier')[1];

    this.expect('{');
    while (!this.peek('}')) {
      if (this.peek('identifier', true, true)) {
        declarations.push(parseGlobalDecl.call(this, this.cur[2]));
      } else if (this.peek('struct', false, true)) {
        structs.push(parseStructDelc.call(this, this.cur[2]));
      } else if (this.peek('method', false, true)) {
        methods.push(parseMethodDecl.call(this, this.cur[2]));
      } else if (this.peek('function', false, true)) {
        functions.push(parseFunctionDecl.call(this, this.cur[2]));
      } else {
