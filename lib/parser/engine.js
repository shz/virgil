...
var Engine = function Engine(src, world) {
  this.world = world;
  this.src = src;
  this.tokens = [];
  this.cur = null;
  ...
  parse: function(allowBare) {
    ...
    while (this.cur[0] != 'eof') {
      ...
      if (!allowBare) {
        ...
        if (stmt instanceof syntax.ImportStatement)
          throw new Error('Imports are invalid when compiling in module mode');
      }
...
module.exports = function(src, world) {
  var engine = new Engine(src, world);
