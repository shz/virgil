...
var calculate = exports.calculate = function(exp) {
  ...
  switch (c) {
    ...
    case syntax.PropertyAccessExpression:
      ...
      if (left.builtin) {
        throw new Error('Parser bug: unable to access properties on built in types.  Soon.');

      // Handle user types
      } else {
        var defScope = exp.scope.search('struct', left.name);
        if (!defScope)
          throw new Error('This should never happen, but there is no struct named "' + left.name + '".  How do we know it\'s a struct?');
        ...
        // TODO - hunt for a method

        throw new Error('Struct "' + left.name + '" has no property or method named "' + exp.right.name + '"');
