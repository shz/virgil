  ...
  switch (node.constructor) {
    ...
    case ast.FunctionCallExpression:
      ...
      if (node.left.type.hasGenericReference()) {
        ...
        var failed = false;
        generics = Object.keys(node.left.type.extractGenerics()).map(function(k) {
          if (resolution[k]) {
            return resolution[k];
          }
          failed = true;
    ...
    case ast.NewExpression:
      ...
      if (node.type.builtin) {
        creation += convertType(node.type, g, true);
      } else {
        creation += makeReference(node.type.def);
        if (node.type.generics.length) {
          creation += '<' + node.type.generics.map(function(t) {
            convertType(t, g, true);
          }).join(', ') + '>';
        }
      }
      creation += '()';
