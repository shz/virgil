...
var makeFunction = function(node, g, isMethod) {
  ...
  if (!node.scope.parent) {
    ...
    var body = node.body && new ast.BlockStatement(node, node.body.body.slice(0));
  ...
  switch (node.constructor) {
    ...
    case ast.ListExpression:
      ...
      return util.expressionize((function() {
        var s = '';
        s += 'auto t = ' + convertType(node.type, 'create_heap', g) + '();';
...
module.exports = function(node, options) {
  ...
  // Put namespace in the front if needed
  if (options.namespace) {
    parts.unshift('namespace ' + options.namespace + ' {\n');
  }

  // Put includes in the front
  ...
  if (node instanceof ast.Module) {
    // Include guard
    var guard = '_VGL_GEN_' + node.filename.toUpperCase().replace(/\W+/g, '_');
    cpp = '#ifndef ' + guard + '\n#define ' + guard + '\n\n' + cpp + '\n\n#endif';

    var ret = {};
    ret[node.filename.replace(/\.vgl$/, '.cpp')] = cpp;
    return ret;
  } else {
    return cpp;
  }
