...
var convertType = function(t, g) {
  ...
  if (t.name == 'list') {
    ...
    return 'std::vector<' + convertType(t.generics[0], g) + '>';
  }
...
var makeFunction = function(node, g, isMethod) {
  ...
  if (!node.scope.parent) {
    ...
    // needed.
    var body = new ast.BlockStatement(node.body.body.slice(0));
    if (isMethod) {
  ...
  switch (node.constructor) {
    ...
    case ast.NewExpression:
      ...
      if (node.args.declarations.length) {
        ...
        s += g.indent(function() {
          var s = '';
          s += node.type.name + ' *temp = ' + creation + ';';
    ...
    case ast.PropertyAccessExpression:
      ...
      if (['list', 'func', 'str'].indexOf(node.left.type.name) >= 0)
        op = '.';

      // Check the property map
      var right = node.right.name;
      if (propertyMap.hasOwnProperty(node.left.type.name))
        if (propertyMap[node.left.type.name].hasOwnProperty(right))
          right = propertyMap[node.left.type.name][right];

      return convert(node.left, g) + op + right;
