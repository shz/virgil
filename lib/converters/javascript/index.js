...
var makeReference = function(node) {
  ...
  var ret = { type: 'Identifier', name: node.name };
  if (node.originModule && node.originModule.length) {
    ret = {
      type: 'MemberExpression',
      computed: false,
      property: ret,
      object: {
        type: 'Identifier',
        name: prefix = '$m' + node.originModule.map(function(s) {
    ...
    case 'ImportStatement':
      ...
      return {
        ...
        declarations: [{
          ...
          id: {
            type: 'Identifier',
            name: '$m' + node.module.map(function(s) {
              return s[0].toUpperCase() + s.substr(1);
            }).join('')
          },
          init: {
            type: 'MemberExpression',
            computed: true,
            object: { type: 'Identifier', name: 'modules'},
            property: { type: 'Literal', value: node.filename }
          }
        }]
      };
      break;
