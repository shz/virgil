...
var renameUnnaturalMethod = function(node) {
  return node.name + '$' + node.args[0][1].toString().replace(/['<>,]/g, '_');
};

var makeFunction = function(node, makeNaturalMethod) {
  var name = node.name;
  var args = node.args;
  ...
  if (node.originModuleNode) {
    ret = {
      ...
      object: {
        type: 'MemberExpression',
        computed: true,
        property: { type: 'Literal', value: node.originModuleNode.path.replace(/\.vgl/, '') },
...
var makeBrowserModule = function(name, exp) {
  var parts = [name.replace(/\.vgl$/, '').replace(/[\/\\]/, '/')]; //name.replace(/\.vgl$/, '').split(path.sep);

  var body = [];
  var cur = { type: 'Identifier', name: '$m' };
  parts.forEach(function(p) {
    cur = {
      type: 'MemberExpression',
      computed: true,
      object: cur,
      property: { type: 'Literal', value: p, raw: '\'' + p + '\'' }
    };
...
module.exports = function(node, options) {
  ...
  if (node instanceof Array) {
    node.forEach(function(node) {
      var n = convert(shadowPass(node), true);
      if (n)
    ...
    for (var i=0; i<node.body.length; i++) {
      var s = convert(shadowPass(node).body[i]);
      if (s)
