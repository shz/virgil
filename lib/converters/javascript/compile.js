...
module.exports = function compile(m) {
  ...
  if (this.needDateTime) {
    body.unshift(this.getDeclarationForImport({
      ast: {
        lib: {
          importPath: ['virgil', 'datetime']
        }
      },
      name: '$DateTime'
    }));
  } else {
    this.needDateTime = tmp;
  }
  ...
  if (isModule) {
    ...
    if (m.lib) {
      ...
      if (filename.match(/\.vgl$/)) {
        filename = filename.replace(/\.vgl$/, '.js');
      } else {
        filename += '.js';
      }
      filemap[path.relative(this.baseDir, filename)] = js;
    }
    return filemap;
  } else {
    return js;
