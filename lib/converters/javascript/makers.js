...
exports.makeReference = function(node) {
  ...
  // that import.
  //
  // There's a bug elsewhere related to imports and scope that's tough
  // to root out, and causes names from the current module to sometimes
  // look like they're coming from a different module.  To work around
  // this temporarily, we'll double check that's not the case.
  // FIXME
  if (node.originModuleNode && node.originModuleNode != this.currentModule) {
    ret = {
      type: 'MemberExpression',
      computed: false,
      property: ret,
