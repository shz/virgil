...
exports.getDeclarationForImport = function(imp) {
  ...
  if (imp.ast.lib) {
    ...
    var outputPath = path.dirname(path.relative(this.baseDir, this.currentModule.filename));
  ...
  return {
    ...
    declarations: [{
      ...
      id: imp.module ? this.getIdentifierForModule(imp.ast) :
                       { type: 'Identifier', name: imp.name },
      init: {
        type: 'CallExpression',
