...
exports.parse = function() {
  ...
  var printUsage = function() {
    ...
    console.log('Usage:', exec, '[-L LIBFOLDER] [-o|--output OUTFILE] [-w|--watch] [-p|--prune ENTRYPOINT] [-d|--debug] [-s|--snippet] [-n|--namespace NAMESPACE] FILE');
    console.log('');
  };
  ...
  for (var i=2; i<process.argv.length; i++) {
    ...
    switch (arg) {
      ...
      // Disabled for now, I done broke it
      // case '--node':
      //   options.node = true;
      //   break;
  ...
  if (!options.module && options.watch) {
    console.error('Watch (-w) is not supported in snippet mode (-s)');
    process.exit(1);
  }
