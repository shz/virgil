...
exports.parse = function() {
  ...
  var printUsage = function() {
    ...
    console.log('Usage:', exec, '[-L|--lib NAME=PATH] [-o|--output OUTFILE] [-w|--watch] [-p|--prune ENTRYPOINT] [-d|--debug] [-n|--namespace NAMESPACE] FILE');
    console.log('');
  };
  ...
  for (var i=2; i<process.argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--lib':
        next = function(arg) {
          var libs = arg.split(',');
          libs.forEach(function(spec) {
            var split = spec.split('=');
            options.libs[split[0]] = split[1];
          });
        };
        break;
  ...
  if (!options.outfile) {
    console.error('Must specify an output directory');
    process.exit(1);
  }
