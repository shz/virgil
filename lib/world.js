...
var processImports = function(w, m, callback) {
  ...
  ast.traverse(m, function(node) {
    if (node.constructor == ast.ImportStatement) {
      var name = node.module.join('.');
      if (toImport.hasOwnProperty(name)) {
        var err = new Error('Double import for module ' + name);
        if (node.loc) {
          err.start = node.loc.start;
          err.end = node.loc.end;
          err.filename = m.path;
          err.src = w.src[m.path];
        }
        return callback(err);
      }
  ...
  async.eachSeries(Object.keys(toImport), function(name, callback) {
    ...
    w.parse(baseDir, name, toImport[name], function(err, m2) {
      //  If, at this point, we failed to *read* the file, then
      // the error originated from the import node and should
      // have its location, src, and filename set from the
      // module we're attempting to compile.
      //
      // We (somewhat crudely) determine if this is the case by
      // looking for a lack of that information on the exception.
      if (err) {
        if (!err.filename) {
          err.filename = m.path;
          err.src = w.src[m.path];
          err.start = toImport[name].loc.start;
          err.end = toImport[name].loc.end;
        }
        return callback(err);
      }

      toImport[name].filename = m2.path;
      callback(undefined, m2);
    });
  }, callback);
};
...
World.prototype.parse = function(root, moduleName, importNode, callback) {
  ...
  findFilename(this.base, fileAttempts, function(err, filename) {
    if (err) {
      ...
      if (this.debug || true) {
        message += ' (Looked for ' + fileAttempts.join('; ') + ')';
      }
