...
World.prototype._loadImport = function(fromModule, importNode, callback) {
  ...
  async.detectSeries(candidates, function(filename, callback) {
    var absFilename = path.resolve(base, filename);
    fs.readFile(absFilename, {encoding: 'utf8'}, function(err, data) {
      if (err) {
    ...
    this._processModule(new ast.Module(null, filename, src), function(err, m) {
      ...
      for (var i in this.options.libs) /* istanbul ignore else */ if (this.options.libs.hasOwnProperty(i)) {
        if (filename.indexOf(this.options.libs[i]) === 0) {
          m.lib = {
            name: i,
            importPath: to.slice(0)
          };
          break;
        }
      }
